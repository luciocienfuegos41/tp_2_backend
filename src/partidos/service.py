...
def get_partidos(equipo=None, fecha=None, fase=None, limit=10, offset=0):
    return repository.get_partidos(
        equipo=equipo, fecha=fecha, fase=fase, limit=limit, offset=offset
    )


def get_partido_by_id(partido_id):
    return repository.get_partido_by_id(partido_id)
...
def actualizar_resultado(partido_id, data):
    ...
    actualizar_resultado = repository.actualizar_resultado(
...
def actualizar_partido_parcial(id_partido, datos):
    ...
    if exito:
        ...
        return {
            "error": "No se pudo actualizar. El partido no existe o hubo un problema en la base de datos"
        }, 404
...
def predecir_partido(id_partido, datos):
    ...
    prediccion = repository.guardar_prediccion(
        id_partido, id_usuario, goles_local, goles_visitante
    )
    if prediccion == 1:
